...
bool Hl1BspLoader::loadEntityData(
    Hl1BspData *data)
{
...
bool Hl1BspLoader::loadSingleEntity(
    Tokenizer &tok,
...
bool Hl1BspLoader::loadTextures(
    Hl1BspData *data,
    IStaticManager *mngr)
{
    ...
    for (int t = 1; t <= textureCount; t++)
    {
        ...
        else
        {
            ...
            if (path.isValid())
            {
                ...
                {
                    fs::byte *bytes = new fs::byte[texture->size()];
                    texture->read(bytes, texture->size());
                    texture->close();
                    delete texture;
                    this->loadMiptex(&textures[t - 1], bytes);
                    delete[] bytes;
                }
            }
            else
